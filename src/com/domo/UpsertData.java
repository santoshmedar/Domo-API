...
public class UpsertData {
	...
	void upload() throws Exception {
		...
		domoApiToken = config.prop.getProperty("domoApiToken");
		...
		if (appendToDomoDataset == true) {
			...
			domo.setkeyColumns(this, manager, dataSetId);
	...
	public static void main(String[] args) {
		...
		if(line != null) {
			...
			if(arguments.get("datasetId")==null)
			{
				System.out.println(new Date().toString() + " datasetId is missing from the arguments list");
				System.exit(1);
			}
			else{ 
				uploadData.dataSetId = arguments.get("datasetId");
				if(arguments.get("keyColumns")==null){
					System.out.println(new Date().toString() + " keyColumns are missing from the arguments list");
					System.exit(1);
				}else{

					uploadData.keyColumns = arguments.get("keyColumns").split(",");
